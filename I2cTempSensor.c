...
Project : Orbit Lab 7 ATE (Temp With UART)
Version : 1.0
Date : 2/20/2013
Author : Brian Zufelt / Craig Kief
Company : COSMIAC/UNM
Comments:
This Lab will pull data from the temperature sensor found on the
...
GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3);
    ...
    GPIOPadConfigSet(GPIO_PORTB_BASE, GPIO_PIN_3, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_OD);         // SDA MUST BE OPEN DRAIN
...
void Read_temp(unsigned char *data){    // Read Temperature sensor
    ...
    unsigned char temp[1];                // storage for data
    ...
    if(data[0] >= 0x33){                        //if temp is greater or equal to 30°C
